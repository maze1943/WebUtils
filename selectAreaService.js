 ...
 * @author 
 * 
 * <input type="text" autocomplete="off" id='city-picker' title="地址" class="inpt" placeholder='请选择省市区县' />
 * selectAreaService.init(data);
		selectAreaService.getSelectArea('city-picker',function(values, displayValues){
			$scope.value=values;
...
(function(window, vx, undefined) {'use strict';
	...
	var services = {};
	services.selectAreaService = [
	function() {
		return {
			init : function(areaInfo) {
				// jshint ignore: start
				+ function(e) {
					e.rawCitiesData = areaInfo;
			...
			getSelectArea : function(id,callback) {
				$("#"+id).cityPicker({
				title : "选择省市区/县",
				onChange : function(picker, values, displayValues) {
					callback(values, displayValues);
				}
			}); 
